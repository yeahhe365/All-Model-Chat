  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--theme-border-secondary)] bg-[var(--theme-bg-primary)]">
        ...
        <button
          onClick={onCancel}
          className={`text-[var(--theme-text-tertiary)] hover:text-[var(--theme-text-primary)] transition-colors rounded-full p-1 ${FOCUS_VISIBLE_RING_PRIMARY_OFFSET_CLASS}`}
        >
          <X size={20} />
        </button>
      </div>
          ...
          <div className="flex flex-col items-center gap-6">
            <div className="relative">
              ...
              <div className="relative w-20 h-20 bg-[var(--theme-bg-accent)]/10 rounded-full flex items-center justify-center text-[var(--theme-text-link)]">
                {isInitializing ? <Loader2 size={40} className="animate-spin" /> : <Mic size={40} />}
              </div>
            </div>
