  ...
  return (
    <div className="bg-[var(--theme-bg-tertiary)]/20 p-3 rounded-xl border border-[var(--theme-border-secondary)]/50">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-0">
            <ToggleItem label={t('settings_filesApi_images')} checked={settings.filesApiConfig.images} onChange={(v) => updateFileConfig('images', v)} small />
            <ToggleItem label={t('settings_filesApi_pdfs')} checked={settings.filesApiConfig.pdfs} onChange={(v) => updateFileConfig('pdfs', v)} small />
            <ToggleItem label={t('settings_filesApi_audio')} checked={settings.filesApiConfig.audio} onChange={(v) => updateFileConfig('audio', v)} small />
            <ToggleItem label={t('settings_filesApi_video')} checked={settings.filesApiConfig.video} onChange={(v) => updateFileConfig('video', v)} small />
            <ToggleItem label={t('settings_filesApi_text')} checked={settings.filesApiConfig.text} onChange={(v) => updateFileConfig('text', v)} small />
        </div>
    </div>
  );
};
