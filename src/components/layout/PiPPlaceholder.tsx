...
export const PiPPlaceholder: React.FC<PiPPlaceholderProps> = ({ onClosePip }) => {
    ...
    return (
        <div className="flex-grow flex flex-col items-center justify-center text-center p-4 bg-[var(--theme-bg-secondary)]">
            ...
            <h2 className="text-xl font-semibold text-[var(--theme-text-primary)]">{t('pip_title')}</h2>
            <p className="text-sm text-[var(--theme-text-secondary)] mt-2 max-w-xs">{t('pip_description')}</p>
            <button 
                onClick={onClosePip} 
