    ...
    return (
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-50 pointer-events-auto max-w-[90vw]">
            <FloatingToolbar className="p-2 gap-2 sm:gap-3">
                ...
                <div className="flex items-center gap-1">
                    <ToolbarButton onClick={onZoomOut} disabled={scale <= 0.4} title={t('imageViewer_zoom_out')}>
                        <ZoomOut size={18} />
                    </ToolbarButton>
                    <ToolbarLabel className="min-w-[40px] text-center px-1">
                        {Math.round(scale * 100)}%
                    </ToolbarLabel>
                    <ToolbarButton onClick={onZoomIn} disabled={scale >= 3.0} title={t('imageViewer_zoom_in')}>
                        <ZoomIn size={18} />
                    </ToolbarButton>
                </div>
