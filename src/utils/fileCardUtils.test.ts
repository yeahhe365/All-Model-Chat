...
describe('getFileCardMeta', () => {
  ...
  it('uses scissors for video configuration outside Gemini 3 mode', () => {
    const meta = getFileCardMeta(createFile({ type: 'video/mp4', name: 'clip.mp4' }), {
      isGemini3: false,
  ...
  it('uses sliders for image/pdf configuration in Gemini 3 mode', () => {
    const imageMeta = getFileCardMeta(createFile({ type: 'image/png', name: 'image.png' }), {
      isGemini3: true,
      includeTextEditing: false,
      requireActiveForConfigure: false,
      canConfigure: true,
    ...
    const pdfMeta = getFileCardMeta(createFile({ type: 'application/pdf', name: 'doc.pdf' }), {
      isGemini3: true,
      includeTextEditing: false,
      requireActiveForConfigure: false,
      canConfigure: true,
  ...
  it('disables configuration when the file is not active and active state is required', () => {
    const meta = getFileCardMeta(createFile({ type: 'video/mp4', uploadState: 'uploading' }), {
      isGemini3: false,
      includeTextEditing: false,
      requireActiveForConfigure: true,
      canConfigure: true,
