...
const buildSuggestionContents = (userContent: string, modelContent: string, language: 'en' | 'zh', fallback = false): StructuredTextContent => {
...
3. 语气自然，符合人类对话习惯。`
    ...
    return [{
        role: 'user',
        parts: [
            { text: fallback ? `${instruction}\n\nReturn the three suggestions as a numbered list, one per line. Do not include any other text or formatting.` : instruction },
...
const buildTitleContents = (userContent: string, modelContent: string, language: 'en' | 'zh'): StructuredTextContent => ([{
    role: 'user',
    parts: [
        {
            text: language === 'zh'
