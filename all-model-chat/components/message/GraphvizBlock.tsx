    ...
    <div className="relative group my-3">
      <div className="flex items-center justify-between px-3 py-2 border border-[var(--theme-border-secondary)] border-b-0 rounded-t-lg bg-[var(--theme-bg-tertiary)]/30 backdrop-blur-sm">
          ...
          <div className="flex items-center gap-1">
             ...
             <button onClick={handleToggleLayout} disabled={isRendering} className={buttonClass} title={`Toggle Layout (Current: ${layout})`}>
                {isRendering ? <Loader2 size={14} className="animate-spin"/> : <Repeat size={14} />}
             </button>
             <button onClick={handleOpenModal} className={buttonClass} title="Expand View">
                <Maximize size={14} />
             </button>
             <button onClick={() => handleDownload('png')} disabled={isDownloading !== 'none'} className={buttonClass} title="Download as PNG">
                {isDownloading === 'png' ? <Loader2 size={14} className="animate-spin"/> : <ImageIcon size={14} />}
             </button>
             <button onClick={() => handleDownload('svg')} disabled={isDownloading !== 'none'} className={buttonClass} title="Download as SVG">
