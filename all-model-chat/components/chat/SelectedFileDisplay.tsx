...
import { Ban, X, AlertTriangleIcon, Loader2, CheckCircle, Copy, Check, FileVideo, FileAudio, FileText, ImageIcon, Youtube, FileCode2 } from 'lucide-react';
import { 
...
export const SelectedFileDisplay: React.FC<SelectedFileDisplayProps> = ({ file, onRemove, onCancelUpload }) => {
  ...
  // Determine if we can show a cancel vs remove button
  ...
  return (
    <div className={`group relative flex flex-col w-24 flex-shrink-0 ${isNewlyActive ? 'newly-active-file-animate' : ''} select-none`}>
      ...
      {/* Close / Cancel Button - Absolute Positioned */}
      <button
        onClick={() => isCancellable ? onCancelUpload(file.id) : onRemove(file.id)}
      ...
      {/* Main Preview Box */}
      <div className="relative w-full aspect-square rounded-xl border border-[var(--theme-border-secondary)] bg-[var(--theme-bg-tertiary)]/30 overflow-hidden flex items-center justify-center transition-colors group-hover:border-[var(--theme-border-focus)]/50">
        ...
        {/* Overlay: Active/Success Animation (Brief) */}
        {isNewlyActive && (
      ...
      <div className="mt-1.5 px-0.5 text-left w-full">
        ...
        <p className="text-[9px] text-[var(--theme-text-tertiary)] truncate leading-tight mt-0.5">
