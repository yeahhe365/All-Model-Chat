...
class GeminiServiceImpl implements GeminiService {
    ...
    async getAvailableModels(apiKeysString: string | null, baseUrl?: string | null): Promise<ModelOption[]> {
    ...
    async getFileMetadata(apiKey: string, fileApiName: string, baseUrl?: string | null): Promise<GeminiFile | null> {
        return getFileMetadataApi(apiKey, fileApiName, baseUrl);
    }

    async generateImages(apiKey: string, modelId: string, prompt: string, aspectRatio: string, abortSignal: AbortSignal, baseUrl?: string | null): Promise<string[]> {
        return generateImagesApi(apiKey, modelId, prompt, aspectRatio, abortSignal, baseUrl);
    }

    async generateSpeech(apiKey: string, modelId: string, text: string, voice: string, abortSignal: AbortSignal, baseUrl?: string | null): Promise<string> {
        return generateSpeechApi(apiKey, modelId, text, voice, abortSignal, baseUrl);
    }

    async transcribeAudio(apiKey: string, audioFile: File, modelId: string, baseUrl?: string | null): Promise<string> {
        return transcribeAudioApi(apiKey, audioFile, modelId, baseUrl);
    }

    async translateText(apiKey: string, text: string, baseUrl?: string | null): Promise<string> {
        return translateTextApi(apiKey, text, baseUrl);
    }

    async generateTitle(apiKey: string, userContent: string, modelContent: string, language: 'en' | 'zh', baseUrl?: string | null): Promise<string> {
        return generateTitleApi(apiKey, userContent, modelContent, language, baseUrl);
    }

    async generateSuggestions(apiKey: string, userContent: string, modelContent: string, language: 'en' | 'zh', baseUrl?: string | null): Promise<string[]> {
        return generateSuggestionsApi(apiKey, userContent, modelContent, language, baseUrl);
    }

    async editImage(apiKey: string, modelId: string, history: ChatHistoryItem[], parts: Part[], abortSignal: AbortSignal, aspectRatio?: string, baseUrl?: string | null): Promise<Part[]> {
