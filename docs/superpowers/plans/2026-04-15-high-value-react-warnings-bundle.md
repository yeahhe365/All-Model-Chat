...
- Later cleanup follow-up work on `main` eliminated the remaining lint backlog from this batch; `npm run lint` is now clean.
- `npm run test` now passes with `70` files and `325` tests green, and the earlier `useChatAreaMessageList must be used within ChatAreaProvider` stderr noise plus the Node `--localstorage-file` warning have both been removed from the test script output.
- The only remaining non-failing console chatter around this area is the upstream Vite/plugin-react deprecation output about `vite:react-babel`, which is not caused by repo code paths in this batch.
